...
import s from './FloMattressChatBot.module.css';
...
const PRESETS = [
  { label: '🛏️ Sizing question',  text: 'What is the weight capacity for a Queen mattress?' },
  { label: '↩️ Trial period',     text: 'How long is the 100-night trial and do I need to keep the box?' },
  { label: '⏱️ Late delivery',    text: 'My order is 12 days late, what is going on?' },
];
...
export default function FloMattressChatBot() {
  ...
  const sessionId = useRef(`flo-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`);
  const bottomRef = useRef<HTMLDivElement>(null);
  const inputRef  = useRef<HTMLInputElement>(null);

  useEffect(() => {
    if (open && messages.length === 0) {
      setMessages([{ id: 'init', role: 'bot', text: "Hi! I'm the Flo Mattress support assistant (demo). How can I help you today?" }]);
  ...
  const send = async (text: string) => {
    ...
    try {
      const res = await fetch('/api/flomattress-chat', {
        method: 'POST',
          ...
          <>
            <div className={s.messages}>
              {messages.map(m => (
                <div key={m.id}>
                  ...
                  {m.escalated && (
                    <div className={s.escalatedBadge}>✉ Escalation raised · check inbox</div>
                  )}
                </div>
              ))}
            ...
            <form className={s.inputRow} onSubmit={onSubmit}>
              <input ref={inputRef} className={s.input} type="text" value={input}
                ...
                placeholder="Ask about your mattress order…" autoComplete="off" />
                ...
                <div className={s.logGrid}>
                  ...
                  <div className={s.logEscalRow}>
                    ...
                    <span className={log.needsEscalation ? s.logEscalYes : s.logEscalNo}>
                      {log.needsEscalation ? '⚠ yes → review queue' : '✓ auto-handled'}
                    </span>
                  </div>
                  {log.emailEscalated && (
                    <div className={s.logEscalRow}>
